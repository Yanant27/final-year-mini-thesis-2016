...
public class LevelOneFailDialog extends JDialog
{
	String playerName="";
	...
	JLabel jlbText=new JLabel("Level One Failed !",SwingConstants.CENTER);
	...
	JLabel jlbTargetScore=new JLabel("Target Score : 25");
	JLabel jlbPlayerScore=new JLabel();
	
	public LevelOneFailDialog(String name, int score)
	{
		this(null,true,name,score);
	}
	public LevelOneFailDialog(Frame parent, boolean modal, String name, int score)
	{
		super(parent,modal);
		setTitle("Level One Fail");
		...
		jbtRetry.addActionListener(new ActionListener()
		{
			...
			{
				setVisible(false);
				LevelOne page=new LevelOne(playerName);
			}
		});
	}
